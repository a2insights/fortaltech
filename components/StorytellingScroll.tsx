  ...
  const storyAtos = [
    {
      ...
      description: "Cabos emaranhados e redes instáveis. Na RMF, o amadorismo tecnológico é o maior gargalo das empresas que tentam escalar, gerando custos invisíveis e frustração diária.",
      bg: "bg-[#0a0f1d]",
    ...
    },
    {
      ...
      description: "Sem governança, seus dados estão a um clique de distância de criminosos cibernéticos. O ransomware não escolhe tamanho de empresa, escolhe as mais despreparadas.",
      bg: "bg-[#0f0505]",
    ...
    },
    {
      ...
      description: "Um software mal gerido ou uma rede que cai interrompe o fluxo de vendas e destrói a produtividade. Cada minuto de tela travada é uma fatia de mercado perdida.",
      bg: "bg-[#000000]",
    ...
    },
    {
      ...
      description: "É aqui que a história muda. Você decide que a tecnologia será o motor do seu crescimento, não o freio. A FortalTech assume o comando para reescrever seu futuro digital.",
    ...
    },
    {
      ...
      description: "Limpamos o caos. Implementamos níveis de acesso, políticas LGPD e firewalls de próxima geração. Seus dados deixam de ser um risco para se tornarem inteligência pura.",
      bg: "bg-[#0a1628]",
    ...
    },
    {
      ...
      description: "Nossa engenharia de campo entrega racks impecáveis, Wi-Fi 6 industrial e redes redundantes. Uma fundação sólida para que seus sistemas nunca parem de rodar.",
      bg: "bg-white",
      darkText: true,
    ...
    },
    {
      ...
      description: "Software sob medida que resolve seus problemas específicos. Nosso laboratório codifica sua vantagem competitiva, integrando hardware e processos em uma interface fluida.",
      bg: "bg-gray-50",
      darkText: true,
    ...
    },
    {
      ...
      description: "Dashboards de telemetria e BI que mostram onde está o lucro. Monitore sua frota e seu consumo de energia em tempo real. A luz definitiva sobre sua gestão.",
      bg: "bg-[#0a1628]",
      accent: "border-[#aa1a20]/30",
      type: "solution"
    },
    {
      ...
      description: "Você foca no seu negócio, nós cuidamos do resto. Monitoramento proativo 24/7. Problemas são mitigados antes mesmo de serem percebidos pela sua equipe.",
      bg: "bg-[#f8fafc]",
      darkText: true,
    ...
    },
    {
      ...
      description: "Não entregamos apenas cabos e código; entregamos liberdade para crescer. Estamos comprometidos em ser a força tecnológica que move o Ceará para o futuro.",
      bg: "bg-white",
      darkText: true,
      isLast: true,
      accent: "border-[#395fa3]/20",
    ...
    const observerCallback = (entries: IntersectionObserverEntry[]) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const index = storyAtos.findIndex(ato => `story-${ato.id}` === entry.target.id);
          if (index !== -1) {
            setActiveStep(index);
          }
        }
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        {storyAtos.map((ato, idx) => {
          ...
          return (
            <div key={ato.id} className="group relative flex items-center justify-end">
              <span className={`absolute right-10 text-[10px] font-black uppercase tracking-widest text-white px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-xl pointer-events-none whitespace-nowrap ${isActive ? 'bg-[#aa1a20]' : 'bg-[#395fa3]'}`}>
                  {ato.title}
              </span>
              <button 
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        style={{ 
          opacity: useTransform(scrollYProgress, [0, 0.5, 1], [0.01, ato.darkText ? 0.03 : 0.06, 0.01]),
          filter: 'blur(100px)'
        }}
      >
            ...
            >
              ...
              {ato.isLast && (
                <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center gap-5">
                  ...
                  >
                    Mudar minha história <Zap className="ml-3 w-5 h-5 group-hover:animate-pulse" />
                  </button>
                </motion.div>
              )}
            </motion.div>
      ...
      <div className={`absolute -bottom-12 -right-12 text-[22vw] font-black opacity-[0.02] select-none leading-none ${ato.darkText ? 'text-slate-600' : 'text-white'}`}>
        0{index + 1}
      </div>
    </section>
  );
