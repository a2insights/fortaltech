...
const ServiceDetailView: React.FC<ServiceDetailViewProps> = ({ serviceSlug, onNavigate }) => {
      ...
      <section className="bg-[#0a1628] pt-40 pb-20 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Voltar ao Ecossistema</span>
          </button>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-3xl">
              <div className="flex items-center space-x-4 mb-6">
                ...
                <div>
                   <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#395fa3]">Especialidade Técnica</span>
            ...
            <div className="hidden lg:block">
               <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem] text-center">
                  <div className="text-4xl font-black text-[#aa1a20] mb-1">24/7</div>
                  <div className="text-[10px] font-black text-white/40 uppercase tracking-widest">Monitoramento Ativo</div>
               </div>
            </div>
          </div>
        </div>
                ...
                {SERVICES.map((s) => (
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-4 transition-colors ${
                      s.slug === serviceSlug ? 'bg-[#395fa3] text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-[#aa1a20] group-hover:text-white'
                    }`}>
                      {React.cloneElement(s.icon as React.ReactElement<any>, { className: "w-4 h-4" })}
                    </div>
                    <span className={`text-xs font-black uppercase tracking-tight ${
                      s.slug === serviceSlug ? 'text-[#395fa3]' : 'text-gray-500 group-hover:text-[#aa1a20]'
                    }`}>
                      {s.title}
                    </span>
                    {s.slug === serviceSlug && <ChevronRight className="ml-auto w-4 h-4 text-[#395fa3]" />}
                  </button>
                ))}
              </nav>
              ...
              <div className="mt-12 p-8 bg-[#0a1628] rounded-[2.5rem] text-white relative overflow-hidden group">
                 <div className="relative z-10">
                    ...
                    <p className="text-blue-100/50 text-[10px] leading-relaxed mb-6 italic">
                      "Desenvolvemos a camada de inteligência que escala sua infraestrutura física para o digital."
                    </p>
                    <a href="#contact" onClick={scrollToContact} className="text-[#aa1a20] font-black text-[10px] uppercase tracking-widest flex items-center hover:text-white transition-colors">
                      Falar com Laboratório <ArrowRight className="ml-2 w-3 h-3" />
                    </a>
                 </div>
          ...
          <main className="lg:w-3/4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    <h3 className="text-xl font-black text-[#395fa3] flex items-center">
                      <Zap className="w-5 h-5 mr-3 text-[#aa1a20]" /> Benefícios da Solução
                    </h3>
                    <div className="space-y-4">
                       {service.benefits.map((benefit, i) => (
                         <div key={i} className="flex items-start bg-gray-50 p-5 rounded-2xl border border-gray-100 group hover:border-[#395fa3] transition-colors">
                            <CheckCircle2 className="w-5 h-5 text-green-500 mr-4 shrink-0 mt-0.5" />
                            <span className="text-gray-700 font-bold text-sm leading-snug">{benefit}</span>
                         </div>
                       ))}
                    </div>
                  </div>
                  <div className="space-y-6">
                    <h3 className="text-xl font-black text-[#395fa3] flex items-center">
                      <Cpu className="w-5 h-5 mr-3 text-[#aa1a20]" /> Especificações Técnicas
                    </h3>
                    <div className="bg-[#0a1628] p-8 rounded-[2.5rem] space-y-4 shadow-xl">
                       {service.technicalSpecs.map((spec, i) => (
                         <div key={i} className="flex items-center space-x-3 text-white/70">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-tight">{spec}</span>
                         </div>
                       ))}
                    </div>
                  </div>
            ...
            <section className="mb-20 py-20 border-y border-gray-100">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {service.sectors.map((sector, i) => (
                    <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-gray-100 flex items-center gap-8 shadow-sm hover:shadow-xl transition-all group">
                       <div className="w-16 h-16 bg-[#395fa3]/5 text-[#395fa3] rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-[#395fa3] group-hover:text-white transition-all">
                          {React.cloneElement(sector.icon as React.ReactElement<any>, { className: "w-8 h-8" })}
                       </div>
                       <div>
                          <h4 className="font-black text-gray-900 text-lg mb-1">{sector.title}</h4>
                          <p className="text-xs text-gray-500 leading-relaxed font-medium">{sector.description}</p>
                       </div>
                    </div>
                  ))}
               </div>
            ...
            <section className="mb-20">
               <div className="bg-gradient-to-br from-[#395fa3] to-[#1a2b4b] p-12 md:p-20 rounded-[4rem] text-white">
                  <h3 className="text-3xl font-black mb-12">Fluxo de Entrega Técnica</h3>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
                     ...
                     {service.workflow.map((step, i) => (
                       <div key={i} className="relative z-10">
                          ...
                          <h5 className="font-black text-sm mb-3 uppercase tracking-tight">{step}</h5>
                       </div>
                     ))}
                  </div>
               </div>
            ...
            <div className="bg-white border-2 border-dashed border-gray-200 rounded-[3rem] p-12 text-center">
               <h4 className="text-2xl font-black text-[#395fa3] mb-4">Sua operação precisa desta infraestrutura?</h4>
               <button 
               ...
               >
                  Solicitar Consultoria <ArrowRight className="ml-3 group-hover:translate-x-2 transition-transform" />
               </button>
            </div>
          </main>
        </div>
