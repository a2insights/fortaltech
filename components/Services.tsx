...
const Services: React.FC<ServicesProps> = ({ onNavigate }) => {
  ...
  const handleNext = useCallback(() => {
    setActiveIndex((prev) => (prev !== null ? (prev + 1) % SERVICES.length : null));
    setProgress(0);
  }, []);

  const handlePrev = useCallback(() => {
    setActiveIndex((prev) => (prev !== null ? (prev - 1 + SERVICES.length) % SERVICES.length : null));
    setProgress(0);
  }, []);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div className="max-w-3xl">
            <div className="inline-flex items-center space-x-2 text-[#aa1a20] mb-4 bg-red-50 px-4 py-1.5 rounded-full border border-red-100">
              ...
              <span className="font-black uppercase tracking-[0.2em] text-[10px]">Expertise de Campo</span>
            </div>
            ...
            <p className="text-gray-600 text-xl leading-relaxed mb-8">
              Enquanto nossa divisão de software cuida da inteligência digital, nossa equipe de campo garante a excelência na infraestrutura física e suporte vital.
            </p>
            ...
            >
              ...
              <span>{isExporting ? 'Gerando Documento...' : 'Baixar Catálogo Completo (PDF)'}</span>
            </button>
          </div>
          <div className="hidden lg:block">
            <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 flex items-center space-x-4 max-w-xs">
              ...
              <div>
                <div className="font-black text-gray-900 leading-tight">99.8% Uptime</div>
                <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Disponibilidade Média</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES.map((service, idx) => (
            ...
            >
              ...
              <h4 className="text-2xl font-black text-[#395fa3] mb-4 group-hover:text-[#aa1a20] transition-colors relative z-10">{service.title}</h4>
              ...
              <div className="space-y-3 mb-10 relative z-10">
                {service.items.slice(0, 3).map((item, i) => (
                  <div key={i} className="flex items-center text-[11px] font-bold text-gray-700 uppercase tracking-tight">
        ...
        <div className="mt-32 bg-[#0a1628] rounded-[4rem] p-10 md:p-20 text-white relative overflow-hidden shadow-2xl border border-white/5">
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="max-w-3xl text-center lg:text-left">
              ...
              <h4 className="text-4xl md:text-6xl font-black mb-8 leading-tight">Precisa de software de alta complexidade?</h4>
              <p className="text-blue-100/60 text-xl leading-relaxed mb-10">
                Desenvolvemos o ecossistema digital que codifica a sua vantagem competitiva. Especialistas em soluções customizadas.
              </p>
            </div>
            <a 
              href="#contact" 
            ...
            >
              Falar com o Laboratório <ArrowRight className="ml-3 w-6 h-6 group-hover:translate-x-2 transition-transform" />
            </a>
          </div>
      ...
      {selectedService && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 bg-[#0a1628]/90 backdrop-blur-xl animate-in fade-in duration-300">
          ...
          <div className="bg-white w-full max-w-7xl rounded-[3rem] shadow-[0_30px_100px_rgba(0,0,0,0.5)] overflow-hidden relative animate-in zoom-in-95 duration-300 flex flex-col h-[85vh]">
            ...
            <div className={`p-6 md:p-8 flex justify-between items-center border-b border-gray-100 ${selectedService.slug.includes('infra') ? 'bg-blue-50/30' : 'bg-red-50/30'}`}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                   <div className="w-12 h-12 bg-white rounded-xl shadow-md flex items-center justify-center text-[#aa1a20]">
                     {React.cloneElement(selectedService.icon as React.ReactElement<any>, { className: 'w-6 h-6' })}
                   </div>
                   <h3 className="text-xl md:text-2xl font-black text-[#395fa3] tracking-tighter">{selectedService.title}</h3>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="hidden md:flex items-center gap-2 mr-4">
                   {SERVICES.map((_, i) => (
                     <div key={i} className={`h-1.5 rounded-full transition-all ${i === activeIndex ? 'w-8 bg-[#aa1a20]' : 'w-2 bg-gray-200'}`}></div>
                   ))}
                </div>
                <button 
                  ...
                  <div className="space-y-10">
                    <div>
                      <h5 className="font-black text-[#395fa3] uppercase text-xs tracking-[0.3em] mb-4 flex items-center">
                        <Zap className="w-4 h-4 mr-2 text-[#aa1a20]" /> Missão Técnica
                      </h5>
                      <p className="text-gray-600 leading-relaxed text-xl md:text-2xl lg:text-3xl font-light">
                        {selectedService.fullDescription.split('.')[0]}. Atuamos com foco em disponibilidade e segurança máxima.
                      </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <div className="space-y-4">
                        <h5 className="font-black text-[#395fa3] uppercase text-[10px] tracking-widest mb-2 border-l-2 border-[#aa1a20] pl-2">Benefícios</h5>
                        <ul className="space-y-3">
                          {selectedService.benefits.slice(0, 3).map((benefit, i) => (
                            <li key={i} className="flex items-start text-xs text-gray-600 font-bold uppercase tracking-tight">
                      ...
                      <div className="space-y-4">
                        <h5 className="font-black text-[#395fa3] uppercase text-[10px] tracking-widest mb-2 border-l-2 border-[#aa1a20] pl-2">Aplicações</h5>
                        <ul className="space-y-3">
                  ...
                  <div className="relative group">
                    ...
                    <div className="relative bg-gray-50 p-10 lg:p-14 rounded-[3.5rem] border border-gray-100 h-full">
                      <h5 className="font-black text-[#395fa3] uppercase text-[10px] tracking-[0.3em] mb-8 flex items-center">
                        <Cpu className="w-4 h-4 mr-2 text-[#aa1a20]" /> Especificações Certificadas
                      </h5>
                      <div className="space-y-6">
                        {selectedService.technicalSpecs.slice(0, 4).map((spec, i) => (
                          <div key={i} className="flex items-center justify-between py-4 border-b border-gray-200">
                            <span className="text-gray-500 font-bold text-xs uppercase tracking-tight">{spec.split(':')[0]}</span>
                            <span className="text-[#395fa3] font-black text-xs">{spec.split(':')[1] || 'ISO COMPLIANT'}</span>
                          </div>
                        ))}
                      </div>
                      <div className="mt-12 p-6 bg-white rounded-3xl border border-blue-100 flex items-center justify-center text-[10px] font-black text-[#395fa3] uppercase tracking-widest shadow-sm">
                         <ShieldCheck className="w-5 h-5 mr-3 text-green-500" /> Ativo em Maracanaú & Grande Fortaleza
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-8 md:p-12 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                >
                  Solicitar Diagnóstico <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
              ...
              <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex md:hidden items-center gap-6">
                   <button onClick={handlePrev} className="p-3 bg-white border border-gray-200 rounded-xl"><ChevronLeft className="w-6 h-6 text-[#395fa3]" /></button>
                   <span className="text-xs font-black text-gray-400">{activeIndex! + 1} / {SERVICES.length}</span>
                   <button onClick={handleNext} className="p-3 bg-white border border-gray-200 rounded-xl"><ChevronRight className="w-6 h-6 text-[#395fa3]" /></button>
                </div>
                ...
                >
                  Documentação Técnica Completa <ChevronRight className="ml-1 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
              </div>
            </div>
          </div>
